# Extending the AMR Stack (ROS 2 Jazzy, C++)
...
```bash
micromamba run -n ros2_jazzy bash -c 'cd /home/cona/kangj/amr_ros2_dev/humble_ws && \
  unset PYTHONPATH && \
...
`ros2_jazzy` env's Python is used (a stray host `PYTHONPATH` breaks the build).
...
- [ ] **Builds + tests pass** under the CONTRACT §8 command
      ...
      `ros2_jazzy` env with `unset PYTHONPATH`).
